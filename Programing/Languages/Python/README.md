...
> [!NOTE]
> 仅 `file`, `mode`, `encoding` 参数常用。

- `file` 要打开的文件的路径（绝对路径或者相对当前工作目录的路径），也可以是要封装文件对应的整数类型文件描述符。
- `mode` 指明文件打开模式的字符串，默认为 `'r'`。可使用下表字符组合，顺序任意，如 `'r+'`, `'w+'`, `'x+'`, `'a+'`, `'rb'`, `'wb'`, `'xb'`, `'ab'`, `'rb+'`, `'wb+'`, `'xb+'`, `'ab+'` 等。

| 字符  | 含义                                             |
| ----- | ------------------------------------------------ |
| `'r'` | 只读，指针在开头                                 |
| `'w'` | 只写，指针在开头，覆盖原内容，文件不存在时创建。 |
| `'x'` | 独占写，指针在开头，文件已存在时报错             |
| `'a'` | 追加写，指针在末尾，文件不存在时创建。           |
| `'b'` | 二进制模式，与其他字符组合                       |
| `'t'` | 文本模式，默认，无需显式指明，与其他字符组合     |
| `'+'` | 读写，与其他字符组合                             |
...
- `buffering` 一个可选的整数，用于设置缓冲策略。
  ...
  - 1，行缓冲（仅在文本模式下可用）。
  - 大于 1 的整数，固定字节大小的块缓冲。
  - 未指定 `buffering` 时，默认的缓冲策略如下：
    - 二进制文件使用固定大小的块缓冲。
    - 交互式文本文件使用行缓冲。
- `encoding` 是用于编码或解码文件的编码格式名称，仅在文本模式下使用。默认为 `locale.getencoding()` 的返回值，建议指定为 `'utf-8'`。
- `errors` 指定如何处理编码和解码错误，不能在二进制模式下使用。
  - `'strict'` 默认值，遇到编码错误引发 `ValueError`。
  - `'ignore'` 忽略错误，可能会导致数据丢失。
  - `'replace'` 将替换标记插入有错误数据的地方。
  - `'surrogateescape'` 将任何不正确的字节表示为 U+DC80 至 U+DCFF 范围内的下方替代码位。
  - `'xmlcharrefreplace'` 编码格式不支持的字符将被替换为相应的 XML 字符引用，仅在写入时支持。
  - `'backslashreplace'` 用 Python 的反向转义序列替换格式错误的数据。
  - `'namereplace'` 用 `\N{...}` 转义序列替换不支持的字符，仅在写入时支持。
- `newline` 决定如何解析来自流的换行符。 它可以为 `None`, `''`, `'\n'`, `'\r'` 和 `'\r\n'`。
  - 读取时
    - 若为 `None`，则启用通用换行模式，输入中的行可以以 `'\n'`，`'\r'` 或 `'\r\n'` 结尾，在返回给调用者之前会被翻译为 `'\n'`。
    - 若为 `''`，则启用通用换行模式，但行尾会被原样返回给调用者。
    - 若为任何其他合法值，则输入行仅由给定的字符串终止，且行尾会被原样返回给调用者。
  - 写入时
    - 若为 `None`，则写入的任何 '\n' 字符都将转换为系统默认行分隔符。
    - 若为 `''` 或 `'\n'`，则不进行转换。
    - 若为任何其他合法值，则写入的任何 '\n' 字符将被转换为给定的字符串。
